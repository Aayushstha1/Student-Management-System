...
const StudentMessages = () => {
  ...
  const createChat = async () => {
    if (!selectedTeacher) {
      setNewChatError('Please select a teacher');
      return;
    }
    try {
      const res = await axios.post('/messages/threads/', { teacher_id: selectedTeacher });
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: 2 }}>
                ...
                {chatList.map((t) => (
                  <ListItem key={t.id} disablePadding>
                    <ListItemButton selected={selectedThread?.id === t.id} onClick={() => handleSelectThread(t)}>
                      <ListItemText
                        primary={t.teacher_name || 'Teacher'}
          ...
          <Paper sx={{ p: 2, height: '70vh', display: 'flex', flexDirection: 'column' }}>
            <Typography variant="subtitle1" sx={{ mb: 1 }}>
              {selectedThread ? `Chat with ${selectedThread.teacher_name}` : 'Select a chat'}
            </Typography>
            <Divider />
            ...
            <List>
              {announceList.map((a) => (
                <ListItem key={a.id} divider>
                  <ListItemText
                    primary={a.title}
                    secondary={`${a.message} • ${a.created_by_name || 'Teacher'} • ${new Date(a.created_at).toLocaleDateString()}`}
                  />
                </ListItem>
              ))}
